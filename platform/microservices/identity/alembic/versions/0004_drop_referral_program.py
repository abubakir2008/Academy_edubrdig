"""identity department: drop the referral program (referral_code,
referred_by_id) — dead code, not the tutor-marketplace pivot. There is no
self-registration endpoint left to enter a code into (every account is
admin-created via /auth/admin/users), `referred_by_id` was never actually
set by any caller, and `get_by_referral_code` had no caller either.
...
SCHEMA = "identity"
_ALPHABET = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789"  # matches the removed crud/user.py::_new_referral_code
